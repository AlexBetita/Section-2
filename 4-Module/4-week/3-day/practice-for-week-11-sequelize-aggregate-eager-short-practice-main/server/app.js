...
app.get('/toys', async (req, res, next) => {
    ...
    // records
    // Your code here
	const toysCount = await Toy.count('price')
...
app.get('/cats/:id/toys', async (req, res, next) => {
	...
	const catToysAggregateData = await Cat.findByPk(req.params.id, {
		...
		attributes: [ // an array of columns/properties
			...
			// key of `toyCount`
			// Your code here
			[sequelize.fn('COUNT', sequelize.col('Toys.price')), 'toyCount'], // two arguments, function, optional <alias>
	...
	// Your code here
	const catData = cat.toJSON() // object

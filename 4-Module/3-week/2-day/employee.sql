CREATE TABLE Employees (
    ID INTEGER PRIMARY KEY AUTOINCREMENT, -- id is unique so its an index
    FirstName VARCHAR(50), -- using the unique property creates an index
    LastName VARCHAR(50),
    Email VARCHAR(100),
    Age INT,
    Position VARCHAR(50),
...
CREATE INDEX employees_first_last_name_index ON Employees(FirstName, LastName);
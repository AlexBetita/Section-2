...
app.get('/instruments/string', async (req, res, next) => {
	const strings = await Instrument.scope(['string', 'defaultScope']).findAll()
    res.json(strings);
});

app.get('/instruments/woodwind', async (req, res, next) => {
	const woodWinds = await Instrument.scope(['woodwind', 'defaultScope']).findAll()
    res.json(woodWinds);
});
...
app.get('/stores/:storeId/instruments', async (req, res, next) => {
	...
	const filterStoreInstruments = await Instrument.
		scope([{ method: ["withStore", storeId] }, 'defaultScope']).
...
app.get('/stores/:storeId/instruments/:type', async (req, res, next) => {
	...
	const filteredInstruments = await Instrument.
		scope([{ method: ["withStore", storeId] }, { method: ["withType", type]}, 'defaultScope']).
		findAll()
